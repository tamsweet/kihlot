            ...
            <div class="col-md-8">
                <div class="box box-primary">
                        ...
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                ...
                                <select required="" name="distype" id="distype" class="form-control">
                                    ...
                                    <option
                                        {{ $coupan->distype == 'fix' ? "selected" : ""}} value="fix">{{ __('adminstaticword.FixAmount') }}</option>
                            ...
                            <div class="form-group">
                                ...
                                <select required="" name="link_by" id="link_by"
                                    ...
                                    <option
                                        {{ $coupan->link_by == 'course' ? "selected" : ""}} value="course">{{ __('adminstaticword.LinktoCourse') }}</option>
                                    <option
                                        {{ $coupan->link_by == 'cart' ? "selected" : ""}} value="cart">{{ __('adminstaticword.LinktoCart') }}</option>
                                    <option
                                        {{ $coupan->link_by == 'category' ? "selected" : ""}} value="category">{{ __('adminstaticword.LinktoCategory') }}</option>
                                </select>
                            ...
                            <div style="{{ $coupan->link_by == 'course' ? "" : "display: none"}}" id="probox"
                                ...
                                <select id="pro_id" name="course_id" class="js-example-basic-single form-control">
                                    ...
                                    @foreach(App\Course::where('status','1')->get() as $product)
                                        @if($product->type == 1)
                                            <option
                                                {{ $coupan->course_id == $product->id ? 'selected' : "" }} value="{{ $product->id }}">{{ $product['title'] }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div style="{{ $coupan->link_by == 'category' ? "" : "display: none"}}" id="catbox"
                                 class="form-group">
                                <label>{{ __('adminstaticword.SelectCategories') }}: <span class="redstar">*</span>
                                </label>
                                <br>
                                <select style="width: 100%" id="cat_id" name="category_id"
                                    ...
                                    @foreach(App\Categories::where('status','1')->get() as $category)
                                        <option
                                            {{ $coupan->category_id == $category->id ? 'selected' : "" }} value="{{ $category->id }}">{{ $category['title'] }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <a href="{{ route('coupon.index') }}" title="Cancel and go back"
                       class="btn btn-md btn-default btn-flat"><i
                            class="fa fa-reply"></i> {{ __('adminstaticword.Back') }}</a>
                </div>
            </div>
        </div>
        </div>
