                        ...
                        <div class="form-group">
                            <form id="demo-form" method="post" action="{{url('relatedcourse/'.$cate->id)}}"
                                ...
                                <div class="row" class="display-none">
                                    <div class="col-md-12">
                                        ...
                                        <select class="display-none" name="main_course_id" class="form-control">
                                            <option
                                                value="{{ $cate->main_course_id }}">{{ $cate->main_course_id }}</option>
                                        </select>
                                    </div>
                                </div>
                                <br>
                                ...
                                <div class="row">
                                    <div class="col-md-8">
                                        ...
                                        <select name="course_id" class="form-control col-md-7 col-xs-12">
                                            @foreach($courses as $cou)
                                                <option
                                                    value="{{ $cou->id }}" {{$cate->course_id == $cou->id  ? 'selected' : ''}}>{{ $cou->title}}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <li class="tg-list-item">
                                            <input class="tgl tgl-skewed" id="cb7"
                                                   type="checkbox" {{ $cate->status==1 ? 'checked' : '' }}>
                                ...
                                <div class="box-footer">
                                    <button type="submit"
                                            class="btn btn-md col-md-2 btn-primary">{{ __('adminstaticword.Submit') }}</button>
                                </div>
