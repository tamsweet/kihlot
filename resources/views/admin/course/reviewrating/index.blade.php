                        ...
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('reviewstatus.quick',$jp->id) }}" method="POST">
                                    {{ csrf_field() }}
                                    <button type="Submit"
                                            class="btn btn-xs {{ $jp->status ==1 ? 'btn-success' : 'btn-danger' }}">
                                        @if($jp->status ==1)
                            ...
                            <td>
                                <form action="{{ route('reviewapproved.quick',$jp->id) }}" method="POST">
                                    {{ csrf_field() }}
                                    <button type="Submit"
                                            class="btn btn-xs {{ $jp->approved ==1 ? 'btn-success' : 'btn-danger' }}">
                                        @if($jp->approved ==1)
                                            {{ __('adminstaticword.Active') }}
                                        @else
                                            {{ __('adminstaticword.Deactive') }}
                            ...
                            <td>
                                <form action="{{ route('reviewfeatured.quick',$jp->id) }}" method="POST">
                                    {{ csrf_field() }}
                                    <button type="Submit"
                                            class="btn btn-xs {{ $jp->featured ==1 ? 'btn-success' : 'btn-danger' }}">
                                        @if($jp->featured ==1)
                                            {{ __('adminstaticword.Active') }}
                                        @else
                                            {{ __('adminstaticword.Deactive') }}
                            ...
                            <td><a class="btn btn-success btn-sm" href="{{url('reviewrating/'.$jp->id)}}"><i
