                ...
                <div class="box box-primary">
                    ...
                    @if($ad->ad_location == "onpause" || $ad->ad_location=="popup")
                        ...
                        <div class="box-header with-border">
                            <h3 class="box-title">Edit AD: {{ $ad->id }} | Location: <span
                                    class="adl">{{ $ad->ad_location }}</span></h3>
                        </div>
                        <div class="panel-body">
                            <form enctype="multipart/form-data" action="{{ route('ad.update.solo',$ad->id) }}"
                                  method="POST">
                                {{ csrf_field() }}
                                {{ method_field('PUT') }}
		                  ...
		                  <strong>{{ $errors->first('ad_image') }}</strong>
                                ...
                                <input class="form-control" type="text" name="ad_target" placeholder="http://"
                                       value="{{ $ad->ad_target }} ">
                                <br>
                    ...
                    @elseif($ad->ad_location == "skip")
                        ...
                        <div class="panel-body">
                            <form action="{{ route('ad.update.video',$ad->id) }}" enctype="multipart/form-data"
                                  method="POST">
                                {{ csrf_field() }}
                                {{ method_field('PUT') }}
	                  ...
	                  <strong>{{ $errors->first('ad_video') }}</strong>
                                ...
                                <br><br>
                                <label for="ad_target">Edit Ad Target: (Click on ad where to redirect user)</label>
                                <input class="form-control" type="text" value="{{ $ad->ad_target }}" name="ad_target"
                                       placeholder="http://">
