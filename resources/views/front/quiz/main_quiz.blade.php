                        ...
                        @else
                            <div id="question_block" class="question-block">
                                <div class="question" id="question-div">
                                    ...
                                    <form action="{{route('start.quiz.store',$topic->id) }}" method="POST"
                                          id="question-form">
                                        {{ csrf_field() }}
                                        <?php
                                        $count = 1;
                                        ...
                                        <div id="more_quiz0">
                                            ...
                                            <div class="jumbotron" id="quiz1">
                                                <h4 style="color:black;">Q{{$count}}
                                                    .&emsp;{{ $que[0]['question'] }}</h4>
                                                <input type="hidden" id="question_id[{{$count}}]"
                                                       name="question_id[{{$count}}]" value="{{ $que[0]['id'] }}">
                                                <input type="hidden" id="canswer[{{$count}}]" name="canswer[{{$count}}]"
                                                       value="{{ $que[0]['answer'] }}">
                                                <label class="radio">
                                                    <input type="radio" required name="answer[{{$count}}]"
                                                           value="A">{{ $que[0]['a'] }}
                                                </label>
                                                <label class="radio">
                                                    <input type="radio" required name="answer[{{$count}}]"
                                        ...
                                        <button style="display: none;" id="prev" title="Click to see previous question"
                                                class="btn btn-md btn-primary" value="1">
                                            << {{ __('frontstaticword.Prev') }}</button>
                                        @if($que_count>1)
                                            <button title="Click to see next question" id="next"
                                                    class="pull-right btn btn-md btn-primary"
                                                    value="0">{{ __('frontstaticword.Next') }} >>
                                            </button>
                                        @else
                                            <button title="Finish the quiz" type="submit"
                                                    class="btn btn-md btn-primary">{{ __('frontstaticword.Finish') }}</button>
                                        @endif
