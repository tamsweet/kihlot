...
<ul id="courseplaylist" class="display-none">
    ...
    @foreach($courses->chapter->sortBy('id') as $chapter)
        ...
        <li data-source="courseplaycontent{{ $chapter->id }}" data-playlist-name="{{ $chapter->chapter_name }}"
            data-thumbnail-path="{{ url('images/course/'.$courses->preview_image) }}">
            <p class="minimalDarkCategoriesTitle"><span
                    class="minimialDarkBold">Title: </span>{{ $chapter->chapter_name }}</p>
            <p class="minimalDarkCategoriesType"><span
                    class="minimialDarkBold">Category: </span>{{ $courses->category->title }}</p>
            <p class="minimalDarkCategoriesDescription"><span
                    class="minimialDarkBold">Description: </span>{{ $courses->short_detail }}</p>
        </li>
                ...
                @if($class->status == '1')
                    <li
                        ...
                        @if($skipads->count()>0)
                            <ul data-ads="">
                                <li @if($skipad->ad_video !="no")
                                    ...
                                    @else
                                    data-source="{{ $skipad->ad_url }}" @endif data-time-start="{{ $skipad->time }}"
                                    data-time-to-hold-ads="{{ $skipad->ad_hold }}"
                                    data-thumbnail-source="{{asset('images/course/'.$chapter->courses->preview_image)}}"
                        ...
                        <div data-video-short-description="">
                            <p class="minimalDarkCategoriesTitle"><span
                                    class="minimialDarkBold">Title: </span>{{ $class->title }}</p>
                            <p class="minimalDarkCategoriesDescription"><span
                                    class="minimialDarkBold">Description: </span>{{ $chapter->courses->short_detail }}
                            </p>
                        </div>
                        ...
                        @if($popupads->count()>0)
                            <div data-add-popup="">
                                <p data-image-path="{{ asset('adv_upload/image/'.$popupad->ad_image) }}"
                                   data-time-start="{{ $popupad->time }}" data-time-end="{{ $popupad->endtime }}"
                                   data-link="{{ $popupad->ad_target }}" data-target="_blank"></p>
                            </div>
                        @endif
                    </li>
                @endif
