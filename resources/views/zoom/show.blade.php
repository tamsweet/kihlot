        ...
        <div class="box">
            ...
            <div class="box-header with-border">
                ...
                <div class="pull-right tools">
                    ...
                    <a title="Edit Meeting" href="{{ route('zoom.edit',$response['id']) }}"
                    ...
                    <div class="modal-dialog modal-sm">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <form method="post" action="{{ route('zoom.delete',$response['id']) }}"
                                      class="pull-right">
                                    {{csrf_field()}}
                                    {{method_field("DELETE")}}


                                    <button type="reset" class="btn btn-gray translate-y-3" data-dismiss="modal">No
                                    </button>
            ...
            <div class="box-body">
                ...
                <hr>
                <h3>Meeting Type :@if($response['type'] == '2') Scheduled Meeting @elseif($response['type'] == '3')
                ...
                <hr>
                <h3>Start Time
                    :{{ isset($response['start_time']) ? date('d-m-Y | h:i:s A',strtotime($response['start_time'])) : "" }}</h3>
                <hr>
                <h3>Meeting Contact Name
                    : {{ isset($response['settings']['contact_name']) ? $response['settings']['contact_name'] : $response['host_email'] }}</h3>
                <hr>
                ...
                <hr>
                <h5><i class="fa fa-microphone" aria-hidden="true"></i> Audio
                    : {{ $response['settings']['audio'] == 'both' ? "Computer and Internet call" : $response['settings']['audio'] }}
                </h5>
                <h5><i class="fa fa-camera" aria-hidden="true"></i> Host Video
                    : {{ $response['settings']['host_video'] == true ? "Enabled" : "Disabled"}}</h5>
                <h5><i class="fa fa-group" aria-hidden="true"></i> Join before Host
                    : {{ $response['settings']['join_before_host'] == true ? "Yes" : "No"}}</h5>
                <h5><i class="fa fa-group" aria-hidden="true"></i> Join before Host
                    : {{ $response['settings']['join_before_host'] == true ? "Yes" : "No"}}</h5>
                <h5><i class="fa fa-group" aria-hidden="true"></i> Participant Video
                    : {{ $response['settings']['participant_video']['join_before_host'] == true ? "Enabled" : "Disabled"}}
                </h5>
            </div>
